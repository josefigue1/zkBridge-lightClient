...
# =============================================================================
# Verify Header 128 Split Circuit - Build and Proof Generation
# =============================================================================
# This script compiles, generates witnesses, trusted setup, and proofs for
# the 8-part split header verification circuits using 128 validators.
#
# RAM Requirements (approximate per part):
#   Part 1A: ~30K constraints    - <1GB
#   Part 1B: ~750K constraints   - ~2GB
...
#
# Usage:
#   ./run_128_split.sh                    # Full pipeline (default)
#   ./run_128_split.sh --compile-only     # Only compile circuits
#   ./run_128_split.sh --witness-only     # Only generate witnesses
#   ./run_128_split.sh --zkey-only        # Only generate trusted setup
#   ./run_128_split.sh --proof-only       # Only generate and verify proofs
#   ./run_128_split.sh --export-verifiers # Export Solidity verifiers
#   ./run_128_split.sh --summary          # Print summary of generated files
#   ./run_128_split.sh --clean            # Remove all build artifacts
#
...
# Output directories
VERIFIER_DIR="$SCRIPT_DIR/verifiers_128"
...
prepare_input() {
    log_step "Preparing 128-validator input..."

    local full_input="$INPUT_DIR/${SLOT}_input.json"
    local input_128="$INPUT_DIR/${SLOT}_input_128.json"

    if [ -f "$input_128" ]; then
        log_info "128-validator input already exists: $input_128"
        return 0
    fi
    ...
    // Extract first 128 validators
    const input128 = {
        signing_root: fullInput.signing_root,
        pubkeys: fullInput.pubkeys.slice(0, 128),
        pubkeybits: fullInput.pubkeybits.slice(0, 128),
        signature: fullInput.signature
    };

    // Validate
    if (input128.pubkeys.length !== 128) {
        console.error('ERROR: Could not extract 128 validators');
        process.exit(1);
    }

    fs.writeFileSync('$input_128', JSON.stringify(input128, null, 2));
    console.log('Created 128-validator input');
    console.log('  signing_root length:', input128.signing_root.length);
    console.log('  pubkeys count:', input128.pubkeys.length);
    console.log('  pubkeybits count:', input128.pubkeybits.filter(b => b === 1).length, 'active');
    "
}
...
generate_witness_part1a() {
    ...
    local input_file="$INPUT_DIR/${SLOT}_input_128.json"
...
generate_all_witnesses() {
    log_header "GENERATING WITNESSES (8 parts)"
...
export_verifiers() {
    ...
    for part in "${PARTS[@]}"; do
        local circuit_name="${CIRCUIT_PREFIX}_${part}"
        local build_dir="$BUILD_DIR/$part"
        local verifier_name="Verifier128_${part}.sol"
...
print_summary() {
    ...
    echo "Configuration:"
    echo "  Mode:       128 validators (8-part split)"
...
print_banner() {
    echo ""
    echo -e "${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║     Verify Header - 128 Validators (8-part split)         ║${NC}"
    echo -e "${BLUE}║     RAM: ~20GB peak per part                              ║${NC}"
    echo -e "${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
main() {
    ...
    echo "Script:     $SCRIPT_DIR/run_128_split.sh"
    echo "Build dir:  $BUILD_DIR"
...
# Run with logging
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/run_128_$(date '+%Y%m%d_%H%M%S').log"
